...
class EntropyBottleneckStanh(EntropyModelSoS):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.num_sigmoids = int(factorized_configuration["num_sigmoids"])
        self.extrema =factorized_configuration["extrema"]
        self.symmetry = factorized_configuration["symmetry"]


        if self.symmetry is False: 
            self.stanh = NonSymStanH(factorized_configuration["beta"],self.num_sigmoids, 
                                     extrema = self.extrema, trainable= factorized_configuration["trainable"])
        else:
            self.stanh = SymStanH(factorized_configuration["beta"],self.num_sigmoids, 
                                  extrema = self.extrema, trainable=factorized_configuration["trainable"])
    ...
    def update(self, device = torch.device("cuda")):
        ...
        average_points = self.stanh.average_points
    ...
    def _probability(self, x, mode = "linear"):
        ...
        levels =  self.stanh.cum_w[:,None].to(x.device) # livelli con dimensioni aumentate
        if mode == "linear":
    ...
    def order_pars(self):
        self.stanh.w = torch.nn.Parameter(torch.sort(self.stanh.w)[0])
        self.stanh.b = torch.nn.Parameter(torch.sort(self.stanh.b)[0])
